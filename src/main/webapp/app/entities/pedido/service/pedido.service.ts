...
type RestOf<T extends IPedido | NewPedido> = Omit<T, 'dataInstant'> & {
...
export class PedidoService {
  ...
  protected convertDateFromClient<T extends IPedido | NewPedido | PartialUpdatePedido>(pedido: T): RestOf<T> {
    return {
      ...pedido,
      dataInstant: pedido.dataInstant?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPedido: RestPedido): IPedido {
    return {
      ...restPedido,
      dataInstant: restPedido.dataInstant ? dayjs(restPedido.dataInstant) : undefined,
    };
  }
